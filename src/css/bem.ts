...
export interface BlockFn {
  (...mods: BemModifier[]): string;
}

export interface ElemFn {
  (elem: string, ...mods: BemModifier[]): string;
}

// Define a more specific type for BEM modifiers
export type BemModifier = string | Record<string, boolean | undefined | null> | BemModifier[];
